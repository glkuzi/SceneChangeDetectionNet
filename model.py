...
def get_siamese_model(input_shape):
    ...
    model = Sequential()
    model.add(Conv2D(16, (10, 10), activation='relu', input_shape=input_shape, #64
                     kernel_regularizer=regularizers.l2(2e-4)))
    model.add(MaxPooling2D(pool_size=(2, 2), padding='same'))
    model.add(Conv2D(8, (7, 7), activation='relu', #128
                     kernel_regularizer=regularizers.l2(2e-4)))
    model.add(MaxPooling2D(pool_size=(2, 2), padding='same'))
    model.add(Conv2D(4, (4, 4), activation='relu', #128
                     kernel_regularizer=regularizers.l2(2e-4)))
    model.add(MaxPooling2D(pool_size=(2, 2), padding='same'))
    ...
    L2_layer = Lambda(lambda tensors:K.sqrt(K.square(tensors[0]) - K.square(tensors[1])) / norm(K.sqrt(K.square(tensors[0]) - K.square(tensors[1])), ord=1))
...
def main():
    ...
    model.compile(loss='cosine_proximity', optimizer=optimizers.Adam(lr=0.001, decay=0.0001), metrics=['accuracy'])
